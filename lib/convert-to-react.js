    ...
    { removeXMLNS: true },
    {
      materialDesign: {
        ...
        fn: function(item) {
          ...
          if (!item.isElem("path")) return true;
...
const getComponentName = iconFileName =>
  camelcase(iconFileName.match(/.*\/(.*)\.svg$/)[1], { pascalCase: true });
...
async function program() {
  ...
  const svgFiles = dirFiles
    .filter(f => f.endsWith(".svg"))
    .map(f => path.resolve(SVG_DIRECTORY, f));

  const progressBar = new Progress(
    "Optimizing icons [:bar] :current/:total :elapsed s",
    {
      complete: "=",
      incomplete: " ",
      width: 20,
  ...
  await pAll(
    svgFiles.map(svgFile => async () => {
      ...
      try {
        ...
        const reactComponentContent = await transformToReactComponent(
          componentName,
          optimizedSvgContent
        );
