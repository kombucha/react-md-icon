...
const svgrOptions = {
  ...
  template: (code, config, state) =>
    `require('./_objectAssignPolyfill');
     var React = require('react');
     module.exports = function ${state.componentName}(${getProps(config)}) { return ${code}; };`
};
...
async function program() {
  ...
  // Prepare folder
  await del([`${DIST_DIRECTORY}/**`, `!${DIST_DIRECTORY}`, `!${DIST_DIRECTORY}/_objectAssignPolyfill.js`]);
